...
class ImprimirEvolucion extends FPDF
{
    /* protected $citologo;
    ...
    public function Header()
    {
        ...
        $this->MultiCell(430, 18, mb_convert_encoding("EVOLUCIÓN", 'UTF-8'), 0, 'C');
        $this->ln(14);
        ...
        $this->Cell(85, 18, iconv('UTF-8', 'windows-1252', "AAB - F - 26 "), 'TB', 0, 'C');
    }
    ...
    public function crearImpresionEvolucion()
    {
        ...
        $this->Cell(50, 18,  mb_convert_encoding('Nombre:', 'UTF-8'), 'TB', 0, 'L');
        $this->SetFont('Arial', '', 9);
        $this->Cell(204, 18,  mb_convert_encoding('SANDRA LILIANA TORIJANO CASTRO', 'UTF-8'), 'TB', 0, 'L');
        // $this->Cell(204, 18, mb_convert_encoding('Nombre:', 'UTF-8'), 0, 0, 'L');
        // $this->ln(18);
        $this->SetFont('Arial', 'B', 9);
        $this->Cell(120, 18,  mb_convert_encoding('Numero de Identificacion:', 'UTF-8'), 'TB', 0, 'L');
        $this->SetFont('Arial', '', 9);
        $this->Cell(182, 18,  mb_convert_encoding('1007412611', 'UTF-8'), 'TB', 0, 'L');
        $this->ln(36);

        $this->SetFont('Arial', 'B', 9);
        $this->Cell(100, 24,  mb_convert_encoding('Fecha', 'UTF-8'), 'TL', 0, 'C');
        $this->Cell(455, 24,  mb_convert_encoding('DESCRIPCIÓN DE LA ATENCIÓN', 'UTF-8'), 'TRL', 0, 'C');
        $this->ln(24);

        $this->SetFont('Arial', '', 9);
        $this->Cell(100, 150,  mb_convert_encoding('11/03/2024', 'UTF-8'), 'TBL', 0, 'C');
        $this->Rect(128, 184, 456, 150);
        $this->Cell(455, 18,  mb_convert_encoding('CONIZACION CERVICAL LLETZ', 'UTF-8'), 0, 0, 'L');
        $this->ln(36);
        $this->Cell(100);
        $this->Cell(200, 12,  mb_convert_encoding('Previa asepsia y anticepsia de region genital', 'UTF-8'), 0, 1, 'L');
        $this->Cell(100);
        $this->Cell(200, 12,  mb_convert_encoding('bajo vision colposcopica se coloca el especulo y se vizualiza cervix identificado zona de transformacion', 'UTF-8'), 0, 1, 'L');
        $this->Cell(100);
        $this->Cell(200, 12,  mb_convert_encoding('se infiltran con lidocaina 4 cuadrantes', 'UTF-8'), 0, 1, 'L');
        $this->Cell(100);
        $this->Cell(200, 12,  mb_convert_encoding('con equipo de radiofrecuencia se realiza cono lletz', 'UTF-8'), 0, 1, 'L');
        $this->Cell(100);
        $this->Cell(200, 12,  mb_convert_encoding('A. CONO EXOCERVIX', 'UTF-8'), 0, 1, 'L');
        $this->Cell(100);
        $this->Cell(200, 12,  mb_convert_encoding('B. CUBO ENDOCERVIX (borde profundo marcado tinta)', 'UTF-8'), 0, 1, 'L');
        $this->Cell(100);
        $this->Cell(200, 12,  mb_convert_encoding('C. LEGRADO EN DOCERVIX RESIDUAL', 'UTF-8'), 0, 1, 'L');
        $this->Cell(100);
        $this->Cell(200, 12,  mb_convert_encoding('Se realiza hemsotasia con asa esferica y percloruro, no complicaciones inmediatas', 'UTF-8'), 0, 1, 'L');
        $this->ln(18);
       /*  $this->SetFont('Arial', 'B', 9);
